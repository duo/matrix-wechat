...
func fnLogin(ce *WrappedCommandEvent) {
	...
	err := ce.User.Connect()
	if err != nil {
		ce.User.log.Error().Msgf("Failed to connect: %s", err)
...
func (u *User) sendQR(ce *WrappedCommandEvent, qrCode []byte) (id.EventID, error) {
	...
	if err != nil {
		u.log.Error().Msgf("Failed to send edited QR code to user: %s", err)
	}
...
func (u *User) uploadQR(ce *WrappedCommandEvent, qrCode []byte) (id.ContentURI, error) {
	...
	if err != nil {
		u.log.Error().Msgf("Failed to upload QR code: %s", err)
...
func fnDeletePortal(ce *WrappedCommandEvent) {
	...
	ce.Portal.log.Info().Str("mxid", ce.User.MXID.String()).Msgf("requested deletion of portal.")
	ce.Portal.Delete()
