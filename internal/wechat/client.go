...
func (wc *WechatClient) LoginWithQRCode() []byte {
	if data, err := wc.requestFunc(wc, &Request{
		...
		wc.log.Warn().Msgf("Failed to login with QR code: %s", err)
		return nil
	} else {
		return data.([]byte)
	}
...
func (wc *WechatClient) IsLoggedIn() bool {
	if data, err := wc.requestFunc(wc, &Request{
		...
		wc.log.Warn().Msgf("Failed to get login status: %s", err)
		return false
	} else {
		return data.(bool)
	}
...
func (wc *WechatClient) GetSelf() *UserInfo {
	if data, err := wc.requestFunc(wc, &Request{
		...
		wc.log.Warn().Msgf("Failed to get self info: %s", err)
		return nil
	} else {
...
func (wc *WechatClient) GetUserInfo(wxid string) *UserInfo {
	if data, err := wc.requestFunc(wc, &Request{
		...
		wc.log.Warn().Msgf("Failed to get user info: %s", err)
		return nil
	} else {
		return data.(*UserInfo)
	}
...
func (wc *WechatClient) GetGroupInfo(wxid string) *GroupInfo {
	if data, err := wc.requestFunc(wc, &Request{
		...
		wc.log.Warn().Msgf("Failed to get group info: %s", err)
		return nil
	} else {
...
func (wc *WechatClient) GetGroupMembers(wxid string) []string {
	if data, err := wc.requestFunc(wc, &Request{
		...
		wc.log.Warn().Msgf("Failed to get group members: %s", err)
		return nil
	} else {
		return data.([]string)
	}
...
func (wc *WechatClient) GetGroupMemberNickname(group, wxid string) string {
	if data, err := wc.requestFunc(wc, &Request{
		...
		wc.log.Warn().Msgf("Failed to get group member nickname: %s", err)
		return ""
	} else {
		return data.(string)
	}
...
func (wc *WechatClient) GetFriendList() []*UserInfo {
	if data, err := wc.requestFunc(wc, &Request{
		...
		wc.log.Warn().Msgf("Failed to get friend list: %s", err)
		return nil
	} else {
		return data.([]*UserInfo)
	}
...
func (wc *WechatClient) GetGroupList() []*GroupInfo {
	if data, err := wc.requestFunc(wc, &Request{
		...
		wc.log.Warn().Msgf("Failed to get group list: %s", err)
		return nil
	} else {
		return data.([]*GroupInfo)
	}
...
func (wc *WechatClient) SendEvent(event *Event) (*Event, error) {
	if data, err := wc.requestFunc(wc, &Request{
		...
		wc.log.Warn().Msgf("Failed to send event: %s", err)
		return nil, err
	} else {
